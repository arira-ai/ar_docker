...
CMD ["echo", "Final winner"]

When you build and run this container, the output will be:
...
```mermaid
graph LR
    A[Start: docker run] --> B{Is ENTRYPOINT defined?}
    
    B -- No --> C{Is CMD defined?}
    C -- No --> D[Error: No command found]
    C -- Yes --> E[Execute CMD]
    
    B -- Yes --> F{Are arguments passed in 'docker run'?}
    F -- No --> G[Execute ENTRYPOINT + CMD]
    F -- Yes --> H[Execute ENTRYPOINT + CLI Arguments]
    H --> I[CMD is ignored/overridden]
```
---
...
sequenceDiagram
    autonumber
    participant Host as Docker CLI/Engine
    participant Engine as Container Runtime
    participant Logic as Command Assembler
    participant PID1 as Container (PID 1)

    Host->>Logic: docker run <image> <args>
    
    Note over Logic: Check Dockerfile + CLI Args
    
    alt CLI Args Provided
        Logic->>Logic: Combine ENTRYPOINT + CLI Args
    else No CLI Args
        Logic->>Logic: Combine ENTRYPOINT + CMD
    end

    Logic->>Engine: Final Execution String
    Engine->>PID1: Start Process
    
    Note right of PID1: Process runs as PID 1<br/>(Graceful signals enabled)
```
